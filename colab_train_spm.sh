...
find "$GOOGLE_DRIVE_PATH/$DATASET_NAME_1/$TRAINING_STRING/" -name '\[라벨\]*.tar.gz' -exec sh -c "cp {} $DATASET_DIR_NAME/$DATASET_NAME_1/$TRAINING_STRING &" \;
find "$GOOGLE_DRIVE_PATH/$DATASET_NAME_2/$TRAINING_STRING/" -name '\[라벨\]*.zip' -exec sh -c "cp {} $DATASET_DIR_NAME/$DATASET_NAME_2/$TRAINING_STRING &" \;
